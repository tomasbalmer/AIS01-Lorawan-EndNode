...
- **Timing fields:** `[ctx+0x38/0x3C/0x40]` are manipulated via bit operations (`bic/orr/lsl`); they represent hardware bitfields/ticks rather than millisecond counters. Actual timing is governed by the peripheral at `0x4001D000`.
- **Persistence:** No Flash/EEPROM writes observed; calibration survives only while RAM + hardware retain state (requires custom persistence in rewritten firmware if desired).
- **AT ↔ Downlink parity:** Both pathways share buffer `0x20004B08` and validators. Final acknowledgement escalates through external routine at `0x080205EA` (outside current dump) invoked by `FUN_080125AC`.
- **Status:** calibration engine fully mapped within available image; outstanding dependency remains external handler at `0x080205EA` (acknowledgement helper).
